...
if "thread_id" not in st.session_state: st.session_state.thread_id = f"thread_{uuid.uuid4().hex[:8]}"
...
status_ph = st.empty()
status_ph.markdown(
    f"**当前事务 Thread ID**: `{st.session_state.thread_id}` | **系统状态**: {st.session_state.system_status}")
st.divider()
...
def start_generation_stream(user_input, chapter_num):
    ...
    status_ph.markdown(f"**当前事务 ID**: `{st.session_state.thread_id}` | **状态**: {st.session_state.system_status}")

    payload = {"user_input": user_input, "thread_id": st.session_state.thread_id, "chapter_num": chapter_num,
                ...
                if line:
                    ...
                    if decoded.startswith("data: "):
                        try:
                            ...
                            # 🌟 核心：流式渲染用 Markdown 替换 Text Area，丝滑不卡顿
