...
async def phase_planner_node(state: dict) -> Dict[str, Any]:
    ...
    # 🌟 核心修改点：抛弃固定的三期数组，修改期数计算逻辑，支持 1卷10期 (100章大跨度)
...
async def chapter_planner_node(state: dict) -> Dict[str, Any]:
    ...
    # 🌟 核心修改点：丢弃了静态的 cliffhanger_rule，改为动态节奏心流引擎
    ...
    sys_prompt = LAYER4_CHAPTER_PROMPT.format(
        ...
        history_context=history_context,
        climax_rule=climax_rule,            # 🌟 注入动态高潮要求
        cliffhanger_rule=cliffhanger_rule   # 🌟 注入动态悬念要求
    )
