...
请严格按以下 JSON 结构输出，禁止包含 Markdown 代码块：
{
  "world_bible": "这里填写世界观、设定、力量体系的完整描述文字（必须是纯字符串，不要嵌套对象）",
  "volumes": [
    {"volume_number": 1, "volume_name": "卷名", "core_goal": "核心任务"},
    ...
  ]
}
"""
...
请必须以严格的 JSON 格式输出，不要包含任何 Markdown 代码块包裹！
"""
...
请严格按以下格式输出 JSON：
{
  ...
  "chapter_title": "章节名",
  "beats": [
    {"plot_summary": "情节摘要", "is_climax": false, "hook": "悬念钩子"}
  ],
  "mandatory_elements": ["本章必须出现的元素1", "元素2"]
}
"""
...
async def book_planner_node(state: dict) -> Dict[str, Any]:
    ...
    structured_llm = llm.with_structured_output(BookOutline, method="json_mode")
    messages = [
...
async def volume_planner_node(state: dict) -> Dict[str, Any]:
    ...
    structured_llm = llm.with_structured_output(VolumePhases, method="json_mode")
    try:
...
async def phase_planner_node(state: dict) -> Dict[str, Any]:
    ...
    structured_llm = llm.with_structured_output(PhaseChapters, method="json_mode")
    try:
...
async def chapter_planner_node(state: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            rewriter_llm = fast_llm.with_structured_output(RAGQueryPlan, method="json_mode")
    ...
    structured_llm = llm.with_structured_output(ChapterOutline, method="json_mode")
